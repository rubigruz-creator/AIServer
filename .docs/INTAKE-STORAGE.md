...
| Функция | Описание |
|---------|----------|
| Сортировка | По дате создания (по умолчанию), **источнику (сайт)**, обновления, сообщений, заявке, телефону, авто, услуге; клик по заголовку |
| Колонка «Источник» | Домен сайта (`source_url`); передаётся из виджета (`?parent=` + URL страницы) |
